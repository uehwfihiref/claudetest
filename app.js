...
function initializeGlobe() {
  if (!window.Globe || !els.globeView) {
    if (els.globeStatus) {
      els.globeStatus.textContent = "3D globe unavailable / using fallback map";
    }
    return;
  }
  ...
  const globe = window.Globe()(els.globeView)
    ...
    .onPolygonHover((feature) => {
      if (els.globeStatus && feature && feature.properties) {
        els.globeStatus.textContent = feature.properties.name;
      } else if (els.globeStatus && uiState.globe.ready) {
        els.globeStatus.textContent = "Globe ready / click a country";
      }
    });
...
async function loadGlobeBorders() {
  ...
  try {
    ...
    els.globeStatus.textContent = `Border dataset failed / ${error.message}`;
