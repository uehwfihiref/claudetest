...
function resolveTurnState(state) {
  ...
  state.units = deriveOrderOfBattle(state.config.country, state.date.getUTCFullYear(), state.historicalIntel);
  return summary;
}
        ...
        .map(
          (source) => `
            <li>
              ...
              <span>${escapeHtml(source.note || "Wikimedia reference")}</span>
            </li>
          `
        )
        .join("")
...
function compileDirectorBriefing(state, stage) {
  ...
  const intel = state.historicalIntel || createFallbackHistoricalIntel(state.config.country, state.date.getUTCFullYear());
  const knownActions = buildKnownForeignActions(state);
  const sourceLine =
    intel.sourceMode === "live"
      ? `Historical source pass surfaced ${intel.sourcedUnitCount} named formations from ${intel.sourceLinks.length} reference page${intel.sourceLinks.length === 1 ? "" : "s"}.`
      : intel.sourceMode === "mixed"
        ? "Historical source pass found partial live coverage and filled the remaining gaps from the local doctrine library."
        : "The historical search layer did not resolve enough live pages, so this packet is leaning on the local doctrine library for structure.";
  const researchLead = intel.notes[0] || `Priority theatres currently look like ${selectedHotspots}.`;
...
function createFallbackHistoricalIntel(country, year) {
  const canonical = canonicalCountry(country);
  return {
    cacheKey: `${canonical}::${year}`,
    country: canonical,
    year,
    sourceMode: "fallback",
    sourcedUnitCount: 0,
    units: generateUnits(country, year),
    notes: [`Historical lookup was unavailable or too thin for ${canonical} in ${year}, so the director fell back to the local doctrine library.`],
    newsWire: [],
    sourceLinks: []
  };
}

function deriveOrderOfBattle(country, year, historicalIntel) {
  const fallback = generateUnits(country, year);
  const merged = {};
...
async function refreshHistoricalIntelForState(state) {
  ...
  const cacheKey = `${canonicalCountry(state.config.country)}::${year}`;
  ...
  if (!shouldUseHistoricalWebResearch(state.config)) {
    ...
    state.units = deriveOrderOfBattle(state.config.country, year, state.historicalIntel);
  ...
  if (!intel) {
    intel = await fetchHistoricalIntel(state.config.country, year);
  ...
  state.units = deriveOrderOfBattle(state.config.country, year, intel);
  return intel;
}

async function fetchHistoricalIntel(country, year) {
  const canonical = canonicalCountry(country);
  ...
  try {
    const branchResults = await Promise.all(activeBranches.map((branch) => researchBranchUnits(canonical, year, branch)));
    const sourcedUnits = {};
    const notes = [];
    const sourceLinks = [];
    ...
    const mergedUnits = deriveOrderOfBattle(country, year, { units: sourcedUnits });
...
async function researchBranchUnits(country, year, branch) {
  const queries = buildBranchQueries(country, year, branch);
  const units = [];
  const sources = [];
  const notes = [];
  ...
  for (const query of queries.slice(0, 4)) {
    ...
    try {
      ...
      console.warn(`Historical search request failed for ${query}.`, error);
      continue;
    }
    ...
    const ranked = results
      .map((result) => ({
        ...result,
        score: scoreHistoricalSearchResult(result, country, year, branch)
      }))
    ...
    for (const result of ranked) {
      ...
      let page = null;
      try {
        page = await fetchWikipediaExtract(result.title);
      } catch (error) {
        console.warn(`Historical extract request failed for ${result.title}.`, error);
      }

      const snippet = stripWikipediaSnippet(result.snippet || "");
      const combinedText = [result.title, snippet, page && page.extract ? page.extract : ""].filter(Boolean).join(". ");
      const pageUnits = extractUnitCandidates(combinedText, branch).filter((candidate) => isLikelyRelevantUnit(candidate, branch, country));
      ...
      if (page && page.title) {
        sources.push({
          title: page.title,
          url: buildWikipediaPageUrl(page.title),
          note: `${branch.label} source`
        });
      } else {
        sources.push({
          title: result.title,
          url: buildWikipediaPageUrl(result.title),
          note: `${branch.label} source`
        });
      }
...
async function searchWikipedia(query, limit) {
  ...
  return (payload && payload.query && payload.query.search) || [];
}
...
async function fetchWikipediaExtract(title) {
  ...
  const page = pages[0];
  return page ? { title: page.title || title, extract: page.extract || "" } : { title, extract: "" };
}
...
function scoreHistoricalSearchResult(result, country, year, branch) {
  const text = `${result.title} ${stripWikipediaSnippet(result.snippet || "")}`.toLowerCase();
  let score = 0;
  country
    .toLowerCase()
    .split(/\s+/)
  ...
  if (/\b(film|album|song|novel|television|football|election)\b/.test(text)) {
    score -= 6;
  }
  return score;
}
...
function isLikelyRelevantUnit(candidate, branch, country = "") {
  ...
  if (/\b(film|album|song|novel|episode|television|wikipedia)\b/.test(text)) {
    return false;
  }
...
function buildBriefingNewsPacket(state, intel) {
  ...
  return dedupeStrings([...liveNews.slice(0, 2), ...incidentNews]).slice(0, 5);
}
